...
const UINT8 USB_DEV_PARA_CDC_MANUFACTURE_STR[] = "TECHPROGENY";
const UINT8 USB_DEV_PARA_VEN_SERIAL_STR[] = "CH-STACK";
const UINT8 USB_DEV_PARA_VEN_PRODUCT_STR[] = "USB2.0 To Serial Port";
const UINT8 USB_DEV_PARA_VEN_MANUFACTURE_STR[] = "TECHPROGENY";
...
void CH341RegWrite(UINT8 reg_add, UINT8 reg_val)
{
  ...
  case 0x18: // SFR_UART_CTRL -->���ڵĲ����Ĵ���
  {
    ...
    CH_LOGD(TAG, "CH341 set para:%d %d %d break:%02x", data_bit_val, (int)stop_bit_val, parity_val, break_en);
  ...
  case 0x27:
  {
    CH_LOGD(TAG, "modem set:%02x", reg_val);
  ...
  // for(i=0; i<USB_IRQ_FLAG_NUM; i++)
  {
    ...
    if (usb_irq_flag[i])
    {
      ...
      case UIS_TOKEN_OUT | 2: // endpoint 2# �����˵��´����
      {
        len = usb_irq_len[i];
        {
          CH_LOGD(TAG, "IN USB EVENT 2[%ld]", len);
          CH_LOG_BUFFER_HEX(TAG, Ep2OUTDataBuf, len);
          // write_function(Ep2OUTDataBuf,len,TRUE);
      ...
      case UIS_TOKEN_OUT | 1: // endpoint 1# �����˵��´����
      {
        len = usb_irq_len[i];
        // Ep1OUTDataBuf
        CH_LOGD(TAG, "IN USB EVENT 1[%ld]", len);
        CH_LOG_BUFFER_HEX(TAG, Ep1OUTDataBuf, len);
        // write_function(Ep1OUTDataBuf,len,TRUE);
        // CH341�������·�
        Ep1DataOUTFlag = 1;
        Ep1DataOUTLen = len;
        VENSer0SendFlag = 1;
        ...
        if (len == sizeof(USB_SETUP_REQ))
        {
          ...
          if ((UsbSetupBuf->bRequestType & USB_REQ_TYP_MASK) == USB_REQ_TYP_VENDOR)
          {
            ...
            case DEF_VEN_DEBUG_WRITE: // д���� 0X9A
            {
              ...
              if ((write_reg_add1 == 0x12) && (write_reg_add2 == 0x13))
              {
                ...
                CH_LOGD(TAG, "CH341 set bps:%d", (int)bps);
                // UART0BpsSet(bps);
              }
              else
              {
          ...
          else if ((UsbSetupBuf->bRequestType & USB_REQ_TYP_MASK) == USB_REQ_TYP_STANDARD)
          {
            ...
            case USB_GET_DESCRIPTOR: // ��ȡ������
            {
              ...
              case 3: // �ַ���������
              {
                CH_LOGD(TAG, "str %d", UsbSetupBuf->wValueL);
                if (usb_work_mode == USB_VENDOR_MODE) // ����ģʽ
                {
                  CH_LOGD(TAG, "str %d", UsbSetupBuf->wValueL);
            ...
            {
              CH_LOGD(TAG, "SET_ADDRESS:%d", UsbSetupBuf->wValueL);
          ...
          else if ((UsbSetupBuf->bRequestType & USB_REQ_TYP_MASK) == USB_REQ_TYP_CLASS)
          {
            ...
            if (data_dir == USB_REQ_TYP_OUT)
            {
              ...
              {
                UINT8 i;
                CH_LOGD(TAG, "SET_LINE_CODING");
                for (i = 0; i < 8; i++)
                {
                  CH_LOGD(TAG, "%02x ", Ep0Buffer[i]);
                }
                CH_LOGD(TAG, "");
                if (Ep0Buffer[4] == 0x00)
                {
              ...
              case DEF_SET_CONTROL_LINE_STATE: /* SET_CONTROL_LINE_STATE */
              {
                ...
                UINT8 present_sta;
                /* ��·״̬ */
                CH_LOGD(TAG, "ctl %02x %02x", Ep0Buffer[2], Ep0Buffer[3]);

                terminal_ctrl = Ep0Buffer[2];
              ...
              default:
              {
                CH_LOGD(TAG, "CDC ReqCode%x", SetupReqCode);
                len = 0xFF; // ����ʧ��
                break;
              }
              }
            ...
            else
            {
              ...
              {
                CH_LOGD(TAG, "GET_LINE_CODING:%d", Ep0Buffer[4]);
                pDescr = Ep0Buffer;
              ...
              {
                CH_LOGD(TAG, "GET_SERIAL_STATE:%d", Ep0Buffer[4]);
                // SetupLen �ж��ܳ���
                len = 2;
                CDCSetSerIdx = 0;
                Ep0Buffer[0] = 0;
        ...
        if (len)
        {
          if (usb_work_mode == USB_CDC_MODE)
          {
            ...
            case DEF_SET_LINE_CODING:
            {
              ...
              CH_LOGD(TAG, "LINE_CODING %d %d %d %d %d", CDCSetSerIdx, (int)set_bps, data_bit, stop_bit, ver_bit);
