...
static inline bool ch_ptr_byte_accessible(const void* ptr) {
    (void) ptr;
    return true;
}

//print number of bytes per line for ch_log_buffer_char and ch_log_buffer_hex
#define BYTES_PER_LINE 16

void ch_log_buffer_hex_internal(const char *tag, const void *buffer, uint16_t buff_len,
                                 ch_log_level_t log_level)
{
    if (buff_len == 0) {
        return;
    }
    ...
    do {
        ...
        if (!ch_ptr_byte_accessible(buffer)) {
        ...
        CH_LOG_LEVEL(log_level, tag, "%s", hex_buffer);
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}

void ch_log_buffer_char_internal(const char *tag, const void *buffer, uint16_t buff_len,
                                  ch_log_level_t log_level)
{
    if (buff_len == 0) {
        return;
    ...
    do {
        ...
        CH_LOG_LEVEL(log_level, tag, "%s", char_buffer);
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}
...
void ch_log_buffer_hexdump_internal(const char *tag, const void *buffer, uint16_t buff_len, ch_log_level_t log_level)
{
    ...
    do {
        ...
        CH_LOG_LEVEL(log_level, tag, "%s", hd_buffer);
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}
