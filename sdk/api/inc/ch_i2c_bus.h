...
typedef struct {
  ...
  union {
    struct {
      uint32_t clk_speed; /*!< I2C clock frequency for master mode, (no higher
                             than 1MHz for now) */
    } master;             /*!< I2C master config */
    struct {
      uint8_t
          addr_10bit_en;   /*!< I2C 10bit address mode enable for slave mode */
      uint16_t slave_addr; /*!< I2C address for slave mode */
      uint32_t maximum_speed; /*!< I2C expected clock speed from SCL. */
    } slave;                  /*!< I2C slave config */
  };
  uint32_t clk_flags; /*!< Bitwise of ``I2C_SCLK_SRC_FLAG_**FOR_DFS**`` for clk
                         source choice*/
} i2c_config_t;
 ...
 *     - ESP_FAIL Fail
 */
int i2c_bus_delete(i2c_bus_handle_t *p_bus_handle);
 ...
 *     - ESP_ERR_TIMEOUT Operation timeout because the bus is busy.
 */
// int i2c_bus_cmd_begin(i2c_bus_device_handle_t dev_handle, i2c_cmd_handle_t cmd);
...
void i2c_bus_write_one_byte(uint8_t dev_addr, uint8_t mem_address,
                            uint8_t data);
